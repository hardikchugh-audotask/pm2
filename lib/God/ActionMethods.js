...
module.exports = function (God) {
    ...
    function fin(err) {
      ...
      if (process_list.length === 0) {
        ...
        if (
          ...
          typeof that.clearDump === "function"
        ) {
  ...
  God.duplicateProcessId = function (id, cb) {
    ...
    if (!God.clusters_db[id] || !God.clusters_db[id].pm2_env)
      return cb(
        God.logAndGenerateError("Error when getting proc || proc.pm2_env"),
        {}
      );
    ...
    if (proc.process && proc.process.pid)
      return cb(
        God.logAndGenerateError(
          "Process with pid " + proc.process.pid + " already exists"
        ),
        {}
      );
  ...
  God.stopProcessId = function (id, cb) {
    ...
    if (!proc.process.pid) {
      console.error(
        "app=%s id=%d does not have a pid",
        proc.pm2_env.name,
        proc.pm2_env.pm_id
      );
  ...
  God.restartProcessId = function (opts, cb) {
    ...
    if (typeof id === "undefined")
      return cb(
        God.logAndGenerateError("opts.id not passed to restartProcessId", opts)
      );
    if (!(id in God.clusters_db))
  ...
  God.restartProcessName = function (name, cb) {
    ...
    eachLimit(
      ...
      function (proc, next) {
        if (God.pm2_being_killed)
          return next(
            "[Watch] PM2 is being killed, stopping restart procedure..."
          );
        if (proc.pm2_env.status === cst.ONLINE_STATUS)
  ...
  God.sendSignalToProcessId = function (opts, cb) {
    ...
    try {
      ...
      return cb(
        God.logAndGenerateError("Error when sending signal (signal unknown)"),
        {}
      );
    }
    return cb(null, God.getFormatedProcesses());
  ...
  God.sendLineToStdin = function (packet, cb) {
    ...
    if (!proc)
      return cb(
        God.logAndGenerateError("Process with ID <" + pm_id + "> unknown."),
        {}
      );
    ...
    if (proc.pm2_env.exec_mode == "cluster_mode")
      return cb(
        God.logAndGenerateError(
          "Cannot send line to processes in cluster mode"
        ),
        {}
      );
    ...
    if (
      ...
      return cb(
        God.logAndGenerateError("Process with ID <" + pm_id + "> offline."),
        {}
      );
  ...
  God.sendDataToProcessId = function (packet, cb) {
    if (
      ...
      return cb(
        God.logAndGenerateError("ID, DATA or TOPIC field is missing"),
        {}
      );
  ...
  God.msgProcess = function (cmd, cb) {
    if ("id" in cmd) {
      ...
      if (action_exist == false) {
        return cb(
          God.logAndGenerateError(
            "Action doesn't exist " + cmd.msg + " for " + proc.pm2_env.name
          ),
          {}
        );
      }
      ...
      return cb(
        God.logAndGenerateError("method requires name or id field"),
        {}
      );
    return false;
  };
